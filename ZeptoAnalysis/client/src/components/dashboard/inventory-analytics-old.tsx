        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {inventoryWeight?.map((weight, index) => {
                const categoryLabel = weight.weightRange.includes('Under') ? 'Very Light' :
                                    weight.weightRange.includes('100g - 500g') ? 'Light' :
                                    weight.weightRange.includes('500g - 1kg') ? 'Medium' :
                                    weight.weightRange.includes('1kg - 5kg') ? 'Heavy' : 'Very Heavy';
                
                const categoryColor = weight.weightRange.includes('Under') ? 'bg-blue-100 text-blue-800' :
                                    weight.weightRange.includes('100g - 500g') ? 'bg-green-100 text-green-800' :
                                    weight.weightRange.includes('500g - 1kg') ? 'bg-yellow-100 text-yellow-800' :
                                    weight.weightRange.includes('1kg - 5kg') ? 'bg-orange-100 text-orange-800' :
                                    'bg-red-100 text-red-800';
                
                return (
                  <tr key={weight.weightRange} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{weight.weightRange}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-lg font-bold text-gray-900">{weight.count.toLocaleString()}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-semibold text-purple-600">{weight.percentage}%</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className="text-xs text-gray-500">{weight.percentage}%</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="chart-container bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
          ...
          <div className="space-y-2">
            <div className="text-sm text-blue-700">
              Under 100g: {inventoryWeight?.find(w => w.weightRange.includes('Under'))?.count || 0} products
            </div>
            <div className="text-sm text-blue-700">
              100g-500g: {inventoryWeight?.find(w => w.weightRange.includes('100g - 500g'))?.count || 0} products
            </div>
        ...
        <div className="chart-container bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200">
          ...
          <div className="space-y-2">
            <div className="text-sm text-orange-700">
              500g-1kg: {inventoryWeight?.find(w => w.weightRange.includes('500g - 1kg'))?.count || 0} products
            </div>
            <div className="text-sm text-orange-700">
              1kg-5kg: {inventoryWeight?.find(w => w.weightRange.includes('1kg - 5kg'))?.count || 0} products
            </div>
        ...
        <div className="chart-container bg-gradient-to-br from-red-50 to-red-100 border-red-200">
          ...
          <div className="space-y-2">
            <div className="text-sm text-red-700">
              Over 5kg: {inventoryWeight?.find(w => w.weightRange.includes('Over'))?.count || 0} products
            </div>
